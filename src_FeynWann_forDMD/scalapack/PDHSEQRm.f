      SUBROUTINE PDHSEQRm( JOB, COMPZ, N, ILO, IHI, H, DESCH, WR, WI, 
     $                  Z, DESCZ, WORK, LWORK, IWORK, LIWORK, INFO )
*
*     ---
*     Modfied by Ravishankar Sundararaman to add intermediate progress
*     October, 2020
*     ---
*
...
*     .. Local Scalars ..
     ...
     $                   HRSRC4, HCSRC4, LIWKOPT
      LOGICAL            INITZ, LQUERY, WANTT, WANTZ, PAIR, BORDER, HEAD
...
*
      ...
      NPROCS = NPROW*NPCOL
      HEAD = (MYROW.EQ.1) .AND. (MYCOL.EQ.1)
...
*
         CALL logPrint(
     $      "   Fixing 2x2 blocks and extracting eigenvalues ... @",
     $      0., .FALSE.)
...
*
      END IF
*
      CALL logPrint("done at t[s]: %.2lf.@", 0., .TRUE.)
      CALL logPrint("   PDHSEQRm @", 0., .FALSE.)
      WORK(1) = LWKOPT
      IWORK(1) = LIWKOPT
      RETURN
*
