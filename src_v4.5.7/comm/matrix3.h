...
public:
	...
	//JX
	void print(FILE* fp, const char *what, int dimension = 3, const char *format = "  %lg", bool brackets = true, bool drop_tiny_off = false, double thr = 1e-2) const{ //!< print to a file / stream
		fprintf(fp, what);
		double diag_min = abs(m[0][0]), off_max = 0;
		for (int i = 0; i < dimension; i++)
		for (int j = 0; j < dimension; j++){
			if (i == j && abs(m[i][i]) < diag_min) { diag_min = abs(m[i][i]); }
			if (i != j && abs(m[i][j]) > off_max) { off_max = abs(m[i][j]); }
		}
		if (drop_tiny_off && off_max < thr * diag_min){
			for (int i = 0; i < dimension; i++){ fprintf(fp, format, m[i][i]); }
			fprintf(fp, "\n");
		}
		else{
			fprintf(fp, "\n");
			for (int i = 0; i < dimension; i++){
				if (brackets) fprintf(fp, "[ ");
				for (int j = 0; j < dimension; j++) fprintf(fp, format, m[i][j]);
				if (brackets) fprintf(fp, " ]\n"); else fprintf(fp, "\n");
			}
		}
	}
	void set(int i, int j, scalar d){
		m[i][j] = d;
	}
	//JX
